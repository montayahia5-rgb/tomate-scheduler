...
# ═════════════════════════════════
_INTRANTS_2026 = {}  # ⚠️ VIDÉ : données prises depuis fichier SOTUSFA uploadé

# ═════════════════════════════════
# AVANCES ET REPORTS RÉELS 2026 — source RECAP El Bourak
# 163 entrées avances | 61 entrées reports
# ═════════════════════════════════
_AVANCES_2026 = {}  # ⚠️ VIDÉ : données prises depuis fichier BOURAK uploadé

_REPORTS_2026 = {}  # ⚠️ VIDÉ : données prises depuis fichier BOURAK uploadé


_PREVISION_2026 = {}  # ⚠️ VIDÉ : données prises depuis fichiers PRÉVISION + BOURAK uploadés
import pandas as pd
import numpy as np
...
def merge_and_calculate(df_bourak, df_royal, df_sotusfa_raw,
    ...
    def _sot_merge(df_raw, df_pivot, base_df):
        ...
        for _, row in base_df.iterrows():
            ...
            # ⚠️ Distribution ACHREF codée DÉSACTIVÉE (utilise SEULEMENT le fichier SOTUSFA)
            # Les membres de centres qui n'ont pas d'achat SOTUSFA direct → 0
            # (leurs intrants sont sur le nom du centre dans le fichier SOTUSFA)
    ...
    with t0:
        ...
        if not _is_admin:
            ...
            if _merged_ok:
                ...
                _role_l = str(CURRENT_ROLE or "").strip().lower()
